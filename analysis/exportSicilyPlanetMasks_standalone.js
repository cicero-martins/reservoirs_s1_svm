/**
 * exportSicilyPlanetMasks_standalone.js
 *
 * Self-contained version of exportSicilyPlanetMasks.js for running from the
 * VS Code Earth Engine extension (or any non-Code-Editor runner). It embeds
 * the AOI geometries, `assetData` and `paperThresholds` directly, since this
 * environment doesn't share state with GEEndwiValidation_app.js the way the
 * browser Code Editor does. Crucially, it contains NO `ui.*` / `Map.*` calls
 * (those only work inside the actual Code Editor's rendered page) -- just the
 * export task submission.
 *
 * Source of truth for assetData/paperThresholds is
 * validation_data/area_Planetscope_data/scripts/GEEndwiValidation_app.js --
...
// --- 3. EXPORT (no ui.*/Map.* -- safe to run outside the Code Editor) ---
...
// Only the 21 complementary-download dates -- everything else was already
// exported/downloaded/integrated in an earlier batch.
var EXPORT_DATES = [
