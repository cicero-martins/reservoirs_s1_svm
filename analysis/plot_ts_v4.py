"""
plot_ts_v4.py  -  Time-series figure for pilot v4 (32 reservoirs, 2014-2021).

  - Grey dots      : JRC rejected (valid_frac < 0.80)
  - Red line+dots  : JRC valid (monthly)
  - Blue/orange pts: SAR raw acquisitions (by pass direction)
  - Dark grey line : SAR cleaned + LOWESS

Dynamic A/P is plotted separately in plot_dyn_ap_v4.py.
32 reservoirs sorted by static A/P ascending.
Output: analysis/method_comparison_output/ts_v4_fullperiod.png
"""

import pathlib
import re as _re
import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import matplotlib.dates as mdates
from matplotlib.lines import Line2D


# ── Clean-and-smooth pipeline ─────────────────────────────────────────────────
def _remove_global(s, threshold=2.0):
    m, sd = s.mean(), s.std()
    return s[np.abs(s - m) <= threshold * sd]

def _remove_local(s, window=5, threshold=1.5):
    arr, idx = s.values.copy(), s.index.tolist()
    keep, half = [], window // 2
    for i in range(len(arr)):
        lo, hi = max(0, i - half), min(len(arr), i + half + 1)
        win = arr[lo:hi]
        m, sd = win.mean(), win.std()
        if sd == 0 or abs(arr[i] - m) <= threshold * sd:
            keep.append(idx[i])
    return s.loc[keep]

def _lowess(dates, values, window_days=20, bandwidth=7):
    smoothed = []
    for t0 in dates:
        diff_d = np.abs((dates - t0).dt.total_seconds().values / 86400)
        mask   = diff_d <= window_days
        w      = np.exp(-(diff_d[mask] / bandwidth) ** 2)
        smoothed.append(float((values[mask] * w).sum() / w.sum()))
    return np.array(smoothed)

def clean_and_smooth(df, col='area_ha'):
    s = df[col].copy()
    s = _remove_global(s, 2.0)
    s = _remove_local(s, 5,  1.5)
    s = _remove_local(s, 5,  1.5)
    s = _remove_local(s, 10, 1.5)
    dates_s  = df.loc[s.index, 'date'].reset_index(drop=True)
    smoothed = _lowess(dates_s, s.reset_index(drop=True), 20, 7)
    return pd.DataFrame({'date': dates_s, 'area_ha': smoothed})


# ── Config ────────────────────────────────────────────────────────────────────
SAR_DIR  = pathlib.Path('raw_data/GEE_GlobalPilotV4b/GEE_GlobalPilotV4')
JRC_DIR  = pathlib.Path('raw_data/GEE_GlobalPilotV4b/GEE_GlobalPilotV4_JRC')
KGE_CSV  = pathlib.Path('analysis/pilot_kge_v4.csv')
OUT_PNG  = pathlib.Path('analysis/method_comparison_output/ts_v4_fullperiod.png')

VALID_FRAC_MIN = 0.80
SAR_MIN_FRAC   = 0.02
DIR_COLOR      = {'ASCENDING': '#1565C0', 'DESCENDING': '#E65100'}

# Per-reservoir minimum area threshold (ha) — excludes partial-polygon acquisitions.
# Saint_Cassien: a bridge bisects the lake; many images only detect the lower section (~200 ha).
AREA_MIN = {'Saint_Cassien': 200}


def _jrc_path(name):
    candidates = sorted(JRC_DIR.glob(f'JRC_area_{name}*.csv'))
    plain = [p for p in candidates if not _re.search(r'\s*\(\d+\)', p.stem)]
    return plain[0] if plain else (candidates[0] if candidates else None)


def load_sar_raw(name):
    p = SAR_DIR / f'SAR_area_{name}.csv'
    if not p.exists():
        return None
    try:
        df = pd.read_csv(p, parse_dates=['date'])
    except pd.errors.EmptyDataError:
        return None
    df = df[df['area_ha'] > 0].sort_values('date').reset_index(drop=True)
    if name in AREA_MIN:
        df = df[df['area_ha'] >= AREA_MIN[name]].copy()
    if df.empty:
        return None
    p99 = df['area_ha'].quantile(0.99)
    df  = df[df['area_ha'] >= SAR_MIN_FRAC * p99].copy()
    m, s = df['area_ha'].mean(), df['area_ha'].std()
    if s > 0:
        df = df[np.abs(df['area_ha'] - m) <= 3.0 * s].copy()
    cols = ['date', 'area_ha', 'passDirection']
    if 'ap_m_dynamic' in df.columns:
        cols.append('ap_m_dynamic')
    return df[cols].sort_values('date').reset_index(drop=True)


def load_jrc(name):
    p = _jrc_path(name)
    if p is None:
        return None, None
    try:
        df = pd.read_csv(p, parse_dates=['date']).sort_values('date')
    except pd.errors.EmptyDataError:
        return None, None
    has_vf   = 'valid_frac' in df.columns
    df_valid = df[df['valid_frac'] >= VALID_FRAC_MIN].copy() if has_vf else df.copy()
    # Sigma-clip: same filter as compute_kge_v4 — removes artefacts passing valid_frac
    m, sd = df_valid['jrc_area_ha'].mean(), df_valid['jrc_area_ha'].std()
    if sd > 0:
        df_valid = df_valid[np.abs(df_valid['jrc_area_ha'] - m) <= 2.5 * sd].copy()
    return df, df_valid


# ── Load KGE table ────────────────────────────────────────────────────────────
kge_df = pd.read_csv(KGE_CSV).dropna(subset=['ap_m', 'kge']).sort_values('ap_m').reset_index(drop=True)
names  = kge_df['name'].tolist()

NCOLS = 5
NROWS = -(-len(names) // NCOLS)   # ceiling division

fig, axes = plt.subplots(NROWS, NCOLS,
                         figsize=(NCOLS * 4.4, NROWS * 2.8),
                         sharex=False)
axes_flat = axes.flatten()

for i, name in enumerate(names):
    ax   = axes_flat[i]
    row  = kge_df[kge_df['name'] == name].iloc[0]
    ap_m = row['ap_m']
    ap_dyn_mean = row.get('ap_m_dynamic', np.nan)
    kge_val = row['kge']

    df_sar          = load_sar_raw(name)
    df_jrc, df_jrcv = load_jrc(name)

    # ── Primary axis (area) ──────────────────────────────────────────────────
    # JRC rejected (grey dots)
    if df_jrc is not None and 'valid_frac' in df_jrc.columns:
        rej = df_jrc[df_jrc['valid_frac'] < VALID_FRAC_MIN]
        if not rej.empty:
            ax.scatter(rej['date'], rej['jrc_area_ha'],
                       s=6, color='#BDBDBD', alpha=0.5, linewidths=0, zorder=1)

    # JRC valid — discontinuous line (NaN gaps where months are missing)
    if df_jrcv is not None and not df_jrcv.empty:
        full_range = pd.date_range('2014-07-01', '2022-03-01', freq='MS')
        jrc_grid   = pd.DataFrame(index=full_range)
        tmp        = df_jrcv.set_index('date')[['jrc_area_ha']]
        tmp.index  = tmp.index.to_period('M').to_timestamp()
        jrc_cont   = jrc_grid.join(tmp)
        ax.plot(jrc_cont.index, jrc_cont['jrc_area_ha'],
                color='#E57373', lw=0.75, alpha=0.65, zorder=2)
        ax.scatter(df_jrcv['date'], df_jrcv['jrc_area_ha'],
                   s=8, color='#C62828', zorder=3, linewidths=0)

    # SAR raw dots
    if df_sar is not None and not df_sar.empty:
        for direction, grp in df_sar.groupby('passDirection'):
            ax.scatter(grp['date'], grp['area_ha'],
                       s=5, color=DIR_COLOR.get(direction, '#666'),
                       alpha=0.35, linewidths=0, zorder=3)

    # SAR cleaned + LOWESS — primary signal, most prominent
    if df_sar is not None and not df_sar.empty:
        c_df = clean_and_smooth(df_sar.reset_index(drop=True))
        if not c_df.empty:
            ax.plot(c_df['date'], c_df['area_ha'],
                    color='#1A237E', lw=1.8, alpha=0.92, zorder=5)

    ax.set_ylabel('Area (ha)', fontsize=5.5, labelpad=2)
    ax.tick_params(axis='y', labelsize=5)

    # ── Labels + formatting ───────────────────────────────────────────────────
    label     = name.replace('_', ' ')
    kge_color = '#1B5E20' if kge_val >= 0.5 else ('#E65100' if kge_val >= 0.0 else '#B71C1C')
    ax.set_title(f'{label}  A/P={ap_m:.0f} m', fontsize=7, fontweight='bold', pad=2)
    ax.text(0.02, 0.97, f'KGE={kge_val:.2f}', transform=ax.transAxes,
            fontsize=7, ha='left', va='top', color=kge_color, fontweight='bold',
            bbox=dict(boxstyle='round,pad=0.2', fc='white', alpha=0.75))

    ax.set_xlim(pd.Timestamp('2014-07-01'), pd.Timestamp('2022-03-01'))
    ax.xaxis.set_major_locator(mdates.YearLocator())
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%Y'))
    ax.tick_params(axis='x', labelsize=5)
    ax.grid(True, alpha=0.2, linewidth=0.4)

for j in range(len(names), len(axes_flat)):
    axes_flat[j].set_visible(False)

legend_handles = [
    Line2D([0],[0], color='#1A237E', lw=2.0, label='SAR LOWESS (primary)'),
    Line2D([0],[0], marker='o', color='w', markerfacecolor='#1565C0',
           markersize=4, label='SAR ascending'),
    Line2D([0],[0], marker='o', color='w', markerfacecolor='#E65100',
           markersize=4, label='SAR descending'),
    Line2D([0],[0], color='#E57373', lw=1.0, marker='o',
           markerfacecolor='#C62828', markersize=3.5, label='JRC valid (reference)'),
    Line2D([0],[0], marker='o', color='w', markerfacecolor='#BDBDBD',
           markersize=4, label='JRC rejected (vf < 0.80)'),
]
fig.legend(handles=legend_handles, loc='lower right',
           bbox_to_anchor=(0.99, 0.01), fontsize=7, framealpha=0.9, ncol=2)

n_shown = len(names)
fig.suptitle(f'SAR vs JRC water area  -  pilot v4, N={n_shown}, 2014–2021  (sorted by A/P asc)',
             fontsize=10, fontweight='bold', y=1.002)
fig.tight_layout(rect=[0, 0.04, 1, 1])
OUT_PNG.parent.mkdir(parents=True, exist_ok=True)
fig.savefig(OUT_PNG, dpi=140, bbox_inches='tight')
plt.close(fig)
print(f'Saved: {OUT_PNG}  ({NROWS}x{NCOLS} grid, {len(names)} panels)')
