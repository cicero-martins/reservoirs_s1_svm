...
These phenomena are well-established individually; yet two linked operational
questions remain unanswered: \emph{which geometric property of a reservoir
best predicts its SAR classification skill, knowable in advance of any SAR
processing, and can that same property tell us which
classifier a reservoir actually needs, separating the cases a simple
single-polarisation threshold already handles from the few that justify a
more complex dual-polarisation learner?}
...
workflow applied to two complementary reservoir samples: a global pilot of
38 reservoirs spanning five continents and a wide range of shoreline
geometry, referenced against the JRC Global Surface Water dataset
\citep{Pekel2016}, and four Sicilian reservoirs validated against
high-resolution PlanetScope imagery as near-truth. We: (i)~quantify the
statistical relationship between A/P and SAR surface-area accuracy across the
global pilot; (ii)~establish, through a like-for-like comparison of a
single-polarisation (VV) per-scene threshold and a dual-polarisation (VV+VH)
per-scene SVM run under an identical pipeline, that per-scene adaptivity
rather than polarisation count is the decisive factor, with dual-polarisation
complexity justified only at low A/P; and
(iii)~audit the computational cost of each classifier, overturning the common
assumption that the dual-polarisation learner is the more expensive option.
Because training samples are drawn automatically from global water masks, the
...
global databases such as GRanD or GROWL before any SAR image is processed, it
lets practitioners anticipate both where the workflow will perform well and
which classifier a given reservoir actually requires.
...
comparison, the computational-cost audit, and the wind analysis.
