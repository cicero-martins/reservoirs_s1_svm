...
\node[stage] (area) at (0,-8.0)   {\textbf{Surface-area series + morphometry}\\dynamic A/P per scene $\cdot$ LOWESS temporal smoothing};
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{sample_selection_pozzillo}
...
maximises this between-class variance is the threshold, which is equivalent
...
(\texttt{ee.data.listOperations}) for all four classifier configurations
(fixed-training dual SVM, per-scene VV Otsu, per-scene dual SVM, and a
no-post-processing fast VV variant), pairing them per reservoir and comparing
cost ratios. The fast configuration also isolates the true cost lever by
removing the post-processing (vectorisation, keep-polygon, and
dynamic-perimeter steps) and computing area by direct pixel counting inside
