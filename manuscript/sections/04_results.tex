...
% four-way N=46 (additionally requires the retired fixed-dual and fast exports,
% never run for the global-coverage/dip-bin/temperate additions -- this N is not
% meant to match the A/P-curve N, see Methods 3.7.1). Figures pulled live from
...
($p = 3\times10^{-5}$; Figure~\ref{fig:ap_curve}). Skill here is the KGE of the
...
(Section~\ref{sec:res_climate}) then determines. The relationship is robust to
the choice of detector (Spearman $0.41$--$0.51$ for the per-scene SVM and VV
Otsu individually) and survives the reference-quality screen by construction,
since the 14 reservoirs with an unreliable optical reference
(Section~\ref{sec:val_global}) never enter this set.
...
\subsection{A simple single-polarisation detector is enough}
\label{sec:res_fourway}

Comparing the four detectors under the identical pipeline, on the 46
reservoirs with all four exports available, overturns the intuitive
``dual-pol beats single-pol'' expectation. Ranked by median KGE, the order is
\emph{per-scene SVM} (0.78) $>$ \emph{per-scene VV Otsu} (0.74) $>$
\emph{VV Otsu without vectorisation} (0.71) $>$ \emph{fixed-training dual SVM}
(0.69) (Figure~\ref{fig:fourway}). Two points follow. First, the
dual-polarisation learner with a fixed 2023 baseline ranks \emph{last}, below
the single-polarisation threshold, because the VV Otsu adapts to every scene
by construction while the fixed SVM does not: per-scene adaptivity matters
more than the number of polarisations, and retraining the SVM per scene is at
worst neutral (median $\Delta$KGE $+0.02$ over the fixed baseline, 23 win /
7 tie / 16 lose), which disposes of the ``why 2023?'' objection.

Second, and more consequentially, the two per-scene detectors are
statistically indistinguishable, and the \emph{single-band} VV Otsu is the
better or equal detector in the majority of reservoirs (it wins 38 of 62 in
Figure~\ref{fig:ap_curve}). A dual-polarisation advantage is not concentrated
anywhere: the difference between the per-scene SVM and the per-scene Otsu does
not depend on A/P
(Spearman$(\text{A/P},\,\text{KGE}_{\text{SVM}}-\text{KGE}_{\text{Otsu}}) =
-0.14$, $p = 0.36$). An earlier, Europe-heavy subsample had suggested that the
second polarisation helps at low A/P; that signal did not survive the global
expansion and we do not claim it. The operational conclusion is parsimony: a
per-scene single-polarisation Otsu threshold is as accurate as the
dual-polarisation SVM across this global set, while being the simplest to
deploy. Removing the vectorisation post-processing (the ``fast'' path), by
contrast, does cost accuracy (median $\Delta$KGE $-0.07$ versus VV Otsu, worse
in 34 of 46 reservoirs), so that step is retained.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{kge_4way}
  \caption{Four-detector KGE across the 46 reservoirs with all four exports.
...
Validated against 3~m PlanetScope near-truth (which breaks the 30~m JRC
inter-product circularity), four Sicilian reservoirs reproduce the main
...
accuracy ordering (Section~\ref{sec:res_fourway}). Combined with the accuracy
result, the recommendation is unambiguous: the per-scene VV Otsu is as accurate
as the per-scene SVM across the global set, is single-band and simple, and costs
roughly 15\% less to run, so it is the method of choice. The most accurate
method on the median, the per-scene SVM, is also the most expensive and buys no
general accuracy gain, so its extra cost is not justified.
...
systematic effect: testing the Bragg prediction that VV should under-detect
water relative to the dual-band SVM as wind rises, on the 46 reservoirs with
wind data and both per-scene detectors, gives
$r(\text{wind}_{p90}, \Delta\text{KGE}) = -0.07$ ($p = 0.67$; Spearman
$\rho = -0.11$, $p = 0.47$; Figure~\ref{fig:wind}), indistinguishable from zero
and far from the correlation Bragg scattering would require. We therefore do not
treat wind as a predictor of which detector is preferable.
...
\begin{figure}[t]
  ...
  for the 46 reservoirs with wind data, coloured by A/P. No relationship is
