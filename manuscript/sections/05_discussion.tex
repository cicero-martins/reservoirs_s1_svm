...
range of geometry and climate, and where it does help, the Sicilian Ancipa
case (Section~\ref{sec:res_sicily}) shows the mechanism is real at an extreme
but does not generalise.

The cost audit points the same way.
...
per-scene single-band Otsu is the simplest and roughly 15\% cheaper to run
...
and we could not tell whether climate mattered or the reference did. We
therefore built an explicit reference-quality screen (a roughness ratio
comparing month-to-month JRC variability to that of two independent SAR
detectors, Section~\ref{sec:val_global}) and excluded the 14 reservoirs it
flagged before
