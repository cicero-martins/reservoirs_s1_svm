...
thresholded per scene, and the same water levels label the resulting waterlines ---
the in-situ gauge by default, or SWOT where the gauge is degenerate over the optical
window (a single source per site, to avoid datum mixing; the choice is made
automatically from the gauge's interquartile range over the mask dates, $<0.5$~m
indicating a stuck sensor). This provides an independent, higher-resolution optical check and, for
