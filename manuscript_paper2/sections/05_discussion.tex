...
$\sim$7\% relative to field truth. Because SWOT observes by swath rather than along a
...
distinct case: its gauge never froze, but independently-measured SAR area
corroborates SWOT's wider water-level range over the gauge's narrower one
(Section~\ref{sec:res_inputvalid}), the highest-A/P reservoir in the set nonetheless
needing the same substitution --- a reminder that gauge reliability and shoreline
geometry are separate axes, not the same risk. For Rosamarina and Pozzillo, the
satellite source was not merely as good as the gauge but strictly better; the reverse
can also hold, however, since at the low-A/P Ancipa the SWOT water level itself
degrades (RMSE $8.8$~m even after datum correction, Section~\ref{sec:res_ladder}) --- so which source is more
reliable is reservoir-dependent, not a general property of SWOT over gauges.
...
Ancipa additionally has a degraded SWOT water level (RMSE $8.8$~m). The same signature
appears in the cross-sensor comparison (Section~\ref{sec:res_xsensor}): the PlanetScope
and SAR reconstructions agree to $1.6$--$1.9$~m at Pozzillo, Poma and Rosamarina but
diverge ($8.5$~m, widening rather than narrowing once new PlanetScope acquisitions filled
an archive gap there --- see Section~\ref{sec:res_xsensor}) at the low-A/P Ancipa, which is
also revisit-rate limited (only 9
...
as a partially-correctable systematic with a $\pm10.3$~pp transfer uncertainty rather
