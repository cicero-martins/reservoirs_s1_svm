...
TSVM.date  = datetime(TSVM.date,  'InputFormat','yyyy-MM-dd');
TADIB.date = datetime(TADIB.date, 'InputFormat','yyyy-MM-dd');


% convert to timetables
TT_svm  = table2timetable(TSVM,  'RowTimes','date');
TT_adib = table2timetable(TADIB, 'RowTimes','date');

% optionally sort and remove duplicate times (average duplicates)
TT_svm = sortrows(TT_svm);
[unqDates,~,ic] = unique(TT_svm.date);
if numel(unqDates) < height(TT_svm)
    avgVol = accumarray(ic, TT_svm.volume, [], @mean);
    TT_svm = timetable(unqDates, avgVol, 'VariableNames',{'volume'});
end

% interpolating

% Use retime to interpolate SVM and Planet at the ADIB times (linear interpolation)
targetDates = TT_adib.date;                 % 1st of month datetimes
...
obs = TT_join.volume_TT_adib;
sim = TT_join.volume_TT_svm_on_adib;
err = sim - obs;
...
metrics = eval_reservoir_model(obs, sim, TT_join.date);
disp(metrics)

% Grafico

figure;
plot(TT_join.date, obs, '-o', 'LineWidth', 1.5, 'MarkerSize', 5);
hold on;
plot(TT_join.date, sim, '-s', 'LineWidth', 1.5, 'MarkerSize', 5);
hold off;
